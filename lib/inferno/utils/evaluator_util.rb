    ...
    module EvaluatorUtil
      ...
      class ReferenceExtractor
        def initialize
          ...
          @resourcetype_ids = Hash.new { |h, k| h[k] = [] }
        ...
        def extract_ids_references(resources)
          resources.each do |resource|
            resource.each_element do |value, metadata, path|
              ...
              @resourcetype_ids[resource.resourceType] << value
          ...
          [@resourcetype_ids, @resource_type_ids, @resource_ids, @references]
        end

        def extract_references(resources)
          resources.each do |resource|
            extract_references_resources(resource)
          end
        end

        def extract_references_resources(resource)
          resource.each_element do |value, metadata, path|
