  ...
  module Utils
    class TlsTester
      ...
      def initialize(params)
        if params[:uri].nil?
          if (params[:host] && params[:port]).nil?
      ...
      def verify_protocol(action = 'verify', ssl_version, readable_version)
        ...
        begin
          ...
          if action == 'deny'
            return true, "Correctly denied connection error of type #{e.class}, message is #{e.message}"
          end
        ...
        [true, "Allowed Connection with #{readable_version}"]
      end
    end
  end
end
