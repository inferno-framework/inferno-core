...
next_action_from_status() {
  ...
  case "$status" in
    ...
    waiting)
      case "$last_test" in
        demo-wait_group-Test02)
          local fail_url
          fail_url=$(printf '%s' "$status_json" \
            | jq -r '.wait_outputs[] | select(.name == "wait_test_url") | .value')
          echo "curl -o /dev/null -s '$fail_url'"
          ;;
        *)
          printf 'Unrecognized wait state. last_test_executed: %s\n' "$last_test" >&2
          return 1
          ;;
      esac
      ;;
