require_relative '../../../dev_suites/dev_multi_patient_api/bulk_data_access/bulk_data_group_export'
...
RSpec.describe MultiPatientAPI::BulkDataGroupExport do
  ...
  let(:access_token) { 'some_access_token_alphanumeric' }
  let(:group_id) { '1219' }
  ...
  describe 'Group export operation confirmation test' do
    let(:runnable) { group.tests[1] }
    let(:export_support) do
    ...
    let(:no_export_support) do
      export_support_json = JSON.parse(export_support)
      export_support_json['rest'][0]['resource'][1]['operation'][0]['definition'] = ''
      export_support_json.to_json
    end

    it 'fails when server does not declare support in CapabilityStatement' do
      stub_request(:get, "#{bulk_server_url}/metadata")
        .to_return(status: 200, body: no_export_support, headers: {})
    ...
    it 'passes when server declares support in CapabilityStatement' do
      stub_request(:get, "#{bulk_server_url}/metadata")
        .to_return(status: 200, body: export_support, headers: {})
  ...
  describe 'Bulk Data Server rejects unauthorized $export test' do
    let(:runnable) { group.tests[2] }

    it 'fails if client can $export without authorization' do
      stub_request(:get, "#{bulk_server_url}/Group/example/$export")
  ...
  describe 'Bulk Data Server returns "202 Accepted" and "Content-location" for $export test' do
    ...
    it 'fails when server return invalid URI via "Content-location" header' do
      stub_request(:get, "#{bulk_server_url}/Group/#{group_id}/$export")
        .with(headers: { 'Authorization' => "Bearer #{access_token}" })
        .to_return(status: 202, body: '', headers: { 'content-location' => 'invalid_uri' })

      result = run(runnable, input)
      expect(result.result).to eq('fail')
      expect(result.result_message).to eq('"invalid_uri" is not a valid URI')
    end

    it 'passes when server returns both "202 Accepted" and "Content-location"' do
      stub_request(:get, "#{bulk_server_url}/Group/#{group_id}/$export")
        .with(headers: { 'Authorization' => "Bearer #{access_token}" })
        .to_return(status: 202, body: '', headers: { 'content-location' => polling_url })
  ...
  describe 'Bulk Data Server returns "202 Accepted" or "200 OK" for status check test' do
    ...
    it 'fails when server returns "200 OK" and invalid response body' do
      stub_request(:get, polling_url.to_s)
        .with(headers: { 'Authorization' => "Bearer #{access_token}" })
        .to_return(status: 200, body: 'invalid_response_body', headers: {})
    ...
    it 'fails when server returns "200 OK" and response body does not contain required attributes' do
      stub_request(:get, polling_url.to_s)
        .with(headers: { 'Authorization' => "Bearer #{access_token}" })
        .to_return(status: 200, body: incomplete_response_body, headers: {})
  ...
  # TODO: Write delete request unit tests after HTTP Client updated to handle
  # deletes.
