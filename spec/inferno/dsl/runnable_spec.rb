...
RSpec.describe Inferno::DSL::Runnable do
  ...
  describe '.minimum_paasing_children' do
    let(:example_suite) do
    ...
    it 'sets and gets minimum passing children' do
      expect(example_suite.minimum_passing_children).to eq([:group1, :group3])
      expect(example_suite.groups[0].minimum_passing_children).to eq([:test2])
      expect(example_suite.groups[1].minimum_passing_children).to eq([])
    end
  end
end
